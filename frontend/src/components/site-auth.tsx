...
const fetchMe = async (cookieHeader: string): Promise<AppUser | null> => {
  try {
    return await apiFetch<AppUser | null>("/auth/me", {
      cache: "no-store",
...
export async function SiteAuth() {
  ...
  const displayName = user.personaName?.trim() || `Steam ${user.steamid64.slice(-4)}`;
